...
namespace SistemsProyect.Components.Pages.Actions.Admin.ViewsButton.AdminCourses.AdminCourses
{
    public partial class AddCourses : UserControl
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                var idTeacher = int.TryParse(ddlProfessors.SelectedValue, out var profId) ? profId : (int?)null;
                var startDate =
                    DateTime.TryParse(txtStartDate.Text, out var start) ? start : (DateTime?)null;
                var endDate = DateTime.TryParse(txtEndDate.Text, out var end) ? end : (DateTime?)null;
                ...
                var result = SubjectOperations.InsertSubject(subject);
                if (result > 0)
                {
